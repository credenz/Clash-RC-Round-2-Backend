...
def usersignup(request):
    if request.method == 'POST':
        try:
            ...
            #making a unique folder for each user
            os.mkdir('ClashRCRound2/users/'+username)
            #now to create question folders:
            '''for i in range(noofquestions):
                   os.mkdir('ClashRCRound2/users/'+username+"/question"+(i+1))   '''
            login(request, user)
